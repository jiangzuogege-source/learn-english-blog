...
cd              切换目录 cd - -->返回上一目录 cd ~ -->用户文件夹  cd / -->根目录
pwd             显示当前目录
...
tail -5 a.txt   显示后5行	head -3 index.html 显示前三行 tail -f index.log 持续监听输入
...
ln -s /user/local/baidu/baidu.sh /usr/bin/baidu     #软连接-快捷方式
```
...
## vim的常用方法
```bash
yum install vim     #安装vim  Esc 普通模式，命令输入
:set nu             #显示行号  :set nonu  :set tabstop=2
                    i 当前插入  I行首插入 a 下一字符 A 行尾 o  下一行 O 上一行 u 撤销
                    dd 剪切 p或P 粘贴  yy 复制  3yy 复制3行 5dd 剪切3行
                    :sh	退回命令行，exit继续进入编辑器 :sp b.txt 新开窗口 ctrl+w 上下键 切换编辑区
                    ! 强制执行命令 :q! :wq! :x!
:w                  b.txt 另存为
:0 :1 :2546         跳转到行 :/qq 查找qq :/n 向下查找 :?f 向上查找
:1,5s/$f/$a/g       从第一行到第五行执行替换，把$f替换为$a，全局替换
 :1,5s/$a/$b/gc     每次提示确定
:r b.txt            把b.txt的数据导入进来
:set autoindent     自动缩进 cd --> vi .vimrc --> set nu set autoindent set tabstop=2
shift + 4           #ESC状态下，跳到行尾，$
shift + 6           #ESC状态下，跳到行首，^
